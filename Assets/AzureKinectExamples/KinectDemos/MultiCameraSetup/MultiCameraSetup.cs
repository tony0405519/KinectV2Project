...
namespace com.rfilkov.components
{
    ...
    public class MultiCameraSetup : MonoBehaviour
    {
        ...
        {
            (int)KinectInterop.JointType.Pelvis, (int)KinectInterop.JointType.ShoulderLeft, (int)KinectInterop.JointType.ShoulderRight,
        ...
        void Update()
        {
                ...
                savedCamPos[0].Count > 0 && cameraOrbit != null /**&& !updatePanel.gameObject.activeSelf*/)
            {
                ...
                if (GetUserPose(0, sensorData, playerIndex, ref tempBodyPose))
                {
            ...
            for (int i = 0; i < numSensors; i++)
            {
                ...
                //Vector3 vUserPos = Vector3.zero; Quaternion qUserRot = Quaternion.identity;
                if (userBodyPose != null && GetUserPose(i, sensorData, playerIndex, ref tempBodyPose))
                {
                    ...
                    for(int j = 0; j < numJoints; j++)
                    {
                        ...
                        if (prevBodyRot == Quaternion.identity)
        ...
        private IEnumerator EstimateCameraPoses()
        {
            ...
            while (!bStopCoroutine)
            {
                ...
                if (kinectManager && kinectManager.IsInitialized() && numSensors > 1 && // bSingleUser &&
                    lastUserTime != 0 && lastUserTime != lastProcUserTime)
                {
                    ...
                    for (int i = 0; i < numSensors; i++)
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            {
                                //if (bodyTransforms != null && bodyTransforms.Length > i && bodyTransforms[i] != null)
                                //{
                                //    bodyTransforms[i].localPosition = refUserPos;
                                //    bodyTransforms[i].localRotation = refUserRot;
                                //}

                                //// src user matrix
                                ////srcUserMatrix = Matrix4x4.TRS(refUserPos, refUserRot, Vector3.one);
                                //srcUserMatrix = Matrix4x4.TRS(userBodyPos[i], userBodyRot[i], Vector3.one);
                        ...
                        else
                        {
                            ...
                            //if (GetUserPose(sensorData, playerIndex, ref userBodyPos, ref userBodyRot))
                            {
                                //if (bodyTransforms != null && bodyTransforms.Length > i && bodyTransforms[i] != null)
                                //{
                                //    bodyTransforms[i].localPosition = userBodyPos;
                                //    bodyTransforms[i].localRotation = userBodyRot;
                                //}

                                //// dst user matrix
                                ////Matrix4x4 dstUserMatrix = Matrix4x4.TRS(userBodyPos, userBodyRot, Vector3.one);
                                //Matrix4x4 dstUserMatrix = Matrix4x4.TRS(userBodyPos[i], userBodyRot[i], Vector3.one);

                                //// dst camera matrix
                                //Matrix4x4 dstCameraMatrix = srcCameraMatrix * srcUserMatrix * dstUserMatrix.inverse;
                                ////Matrix4x4 dstCameraMatrix = srcUserMatrix * dstUserMatrix.inverse;  // Chris
        ...
        private bool GetUserPose(int sIndex, KinectInterop.SensorData sensorData, int uIndex, ref UserBodyPose userPose)
        {
        ...
        private void SaveSensorPose(int sIndex, Vector3 camPos, Quaternion camRot, ref UserBodyPose userPose)
        {
            ...
            if (savedCamPos[sIndex].Count == savedCamRot[sIndex].Count)
            {
                ...
                float fPoseError = (bEnoughSamples && sIndex != 0) ? GetCumulativePoseError(sIndex, mvaCameraPos, mvaCameraRot, fromSavedPR, numSavedPR) : float.MaxValue;
        ...
        // returns the cumulative user pose error with the given camera pose
        private float GetCumulativePoseError(int sIndex, Vector3 cameraPos, Quaternion cameraRot, int fromPose, int toPose)
        {
            if (sIndex == 0)
