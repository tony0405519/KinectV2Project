...
namespace com.rfilkov.components
{
    ...
    public class CubemanController : MonoBehaviour
    {
        ...
        [Tooltip("Whether the cubeman is in world coordinates or in the sensor's coordinate system.")]
        public bool worldCoordSystem = true;
        ...
        void Update()
        {
            ...
            Vector3 posPointMan = worldCoordSystem ? kinectManager.GetUserPosition(userID) : kinectManager.GetUserKinectPosition(userID, true);
            ...
            for (int i = 0; i < bones.Length; i++)
            {
                if (bones[i] != null)
                {
                    ...
                    if (kinectManager.IsJointTracked(userID, joint))
                    {
                        ...
                        Vector3 posJoint = worldCoordSystem ? kinectManager.GetJointPosition(userID, joint) : kinectManager.GetJointKinectPosition(userID, joint, true);
